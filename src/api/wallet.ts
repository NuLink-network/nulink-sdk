import {
  ...
  getPolicyLabelIdsByAccountId,
} from "../core/pre";
import {
  isBlank,
  UserDataCorruptedError,
  UserDataVersionLowError,
  Account,
  NuLinkHDWallet,
} from "../core";
 ...
 */
export const checkUserDataVersionMatch = async (
  ...
  try {
    const dataInfo = await NuLinkHDWallet.parseUserDataVersionInfo(
    ...
    const data = (await getPolicyLabelIdsByAccountId(
      accountIds[0]
    )) as object;
    ...
    return (
      /* serverSortedStrategyIds */ serversStrategyIds.filter(
        (x) => !localSet.has(x)
      ).length === 0
    ); // difference set:  a - b  ==> serverSortedStrategyIds - localSet must be []
  } catch (error) {
...
export const restoreWalletDataByRootExtendedPrivateKey = async (
  ...
  const wallet: NuLinkHDWallet =
    await NuLinkHDWallet.restoreWalletDataByRootExtendedPrivateKeyAndServerStrategyInfos(
      newPassword,
      rootExtendedPrivateKey
    );
  const defalutAccount: Account = wallet
    .getAccountManager()
...
/*  restore account by the strategys of account stored in the backend db. 
    ...
    we can't export the extended private key, we need to export the root extended private key and the index information of all accounts
*/
export const restoreWalletDataByMnemonic = async (
  newPassword: string,
  mnemonic: string
): Promise<NuLinkHDWallet> => {
  const wallet: NuLinkHDWallet =
    await NuLinkHDWallet.restoreWalletDataByMnemonicAndServerStrategyInfos(
      newPassword,
      mnemonic
    );
  const defalutAccount: Account = wallet
 ...
 * @throws {UserDataCorruptedError, UserDataVersionLowError}
 */
export const restoreWalletData = async (
  newPassword: string,
  dataFileBinaryString: string
): Promise<NuLinkHDWallet> => {
  ...
  const wallet: NuLinkHDWallet = await NuLinkHDWallet.restoreHDWalletByData(
    newPassword,
    jsonData
  );
  const defalutAccount: Account = wallet
...
export const loadWallet = async (
  password?: string
): Promise<NuLinkHDWallet | null> => {
  const nuLinkHDWallet: NuLinkHDWallet | null =
    await NuLinkHDWallet.loadHDWallet(password);
 ...
 */
export const getWalletDefaultAccount = async (
  password?: string
 ...
 * @return {*}
 */
export const getDefaultAccountPrivateKey = async (
  password: string
): Promise<string | null> => {
  const nuLinkHDWallet = await NuLinkHDWallet.loadHDWallet(password);
