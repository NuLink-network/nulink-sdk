...
const PRIVODER_NAME = (account: 'Account') => `${INFURA_WEB3PROVIDER_INSTANCE_NAME_PREFIX}_${(account as any).address}`;
const INFURAPROVIDER_INSTANCE_NAME = "infuraprovider";
...
export const getInfuraProvider = async (ethUrl?: string): Promise<ethers.providers.InfuraProvider> => {
  // for get instance with saved key
  let infuraProvider = SingletonService.get<ethers.providers.InfuraProvider>(INFURAPROVIDER_INSTANCE_NAME);
...
export const setInfuraProvider = async (infuraUrl: string): Promise<ethers.providers.InfuraProvider> => {
  ...
  SingletonService.set<ethers.providers.InfuraProvider>(INFURAPROVIDER_INSTANCE_NAME, infuraProvider, true);
  return infuraProvider;
};
