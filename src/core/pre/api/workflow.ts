...
/**
 * Uploads files/data to the server by selecting an existing policy (note: policy has create on chain) and uploading the files/data encrypted with the policy's public key to IPFS.
 * @category Data Publisher(Alice) Upload Data
...
export const estimatePolicyGas = async (
  ...
  if (approveStatus === 2) {
    ...
    throw new PolicyHasBeenActivedOnChain('Policy is approved, no need apply again');
  } else if (approveStatus == 4) {
    throw new PolicyApproving('Policy is under review, please wait for the review to complete');
  }
...
export const approvalApplicationForUseData = async (
  ...
  do {
    try {
      ...
      console.error(`getTransactionReceipt createPolicy txHash: ${enPolicy.txHash} retrying, error: `);
    }

    if (isBlank(receipt)) {
      if (retryTimes % 3 == 0) {
        //Message.info(
        console.log('Transaction is being confirmed on the blockchain. Please wait patiently');
      }
      await sleep(3000);
    }
    retryTimes--;
